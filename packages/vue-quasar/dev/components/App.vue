  ...
  <div class="container">
    ...
    <aside class="example-selector">
      <div class="data">
        ...
        <details>
          ...
          <pre>{{ JSON.stringify(example.schema, null, 2) }}
          </pre>
        </details>

        <details>
          <summary>uischema</summary>
          <pre>{{ JSON.stringify(example.uischema, null, 2) }}
          </pre>
        </details>
    ...
    <div class="tools">
      ...
      <select v-model="currentExampleName" @change="onExampleChange($event)">
        <option v-for="option in examples" :key="option.name" :value="option.name" :label="option.label">
          {{ option.label }}
        </option>
      </select>
    </div>
    ...
    <main class="form">
      <article>
        <json-forms :key="example.name" :data="example.data" :schema="example.schema" :uischema="example.uischema"
          :renderers="renderers" :i18n="example.i18n" :additional-errors="additionalErrors" @change="onChange">
        </json-forms>
      </article>
    </main>
  </div>
